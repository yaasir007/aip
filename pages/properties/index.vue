        ...
        >
          ...
          Advanced Filter
			...
			<div v-if="filteredProperties.length === 0 && !isLoading" class="text-center mt-8">
				<p class="text-xl text-gray-600">No properties found matching your search criteria.</p>
			</div>
		</main>
	</div>
</template>
...
const filteredProperties = computed(() => {
	...
	return properties.value.filter(
		(property) =>
			property.name.toLowerCase().includes(searchQuery.value.toLowerCase()) || property.address.toLowerCase().includes(searchQuery.value.toLowerCase())
	);
});
</script>
